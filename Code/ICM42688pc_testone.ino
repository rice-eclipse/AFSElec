...
void imuReadRegs(uint8_t startReg, uint8_t *buf, uint8_t len) {
  SPI1.beginTransaction(imuSPISettings);
  digitalWrite(IMU_CS, LOW);
  // Serial.print(imuReadReg(0x2E), BIN);
  Serial.print(", (before delay) ");
  Serial.println(imuReadReg(0x2D),HEX);
  delayMicroseconds(15);
  // // Serial.print(imuReadReg(0x2E), BIN);
  // Serial.print("(after delay) ");
  // Serial.println(imuReadReg(0x2D),BIN);
...
void imuReadAll() {
  ...
  // 1. Poll for Avail/Locked (as per flowchart)
  uint32_t timeout = micros();
  while (micros() - timeout < 5000) { // 5ms timeout
    statusInt = imuReadReg(0x2D); // STATUSINT
    if ((statusInt & 0x03) == 0x03) break; // Both Avail and Locked are set
  }

  // If it's stuck at 0x00, the sensor isn't updating.
  if ((statusInt & 0x03) != 0x03) {
    // DEBUG: Force a status clear if we are stuck
    Serial.println(imuReadReg(0x2E)); 
    return; 
  }
  ...
  SPI1.beginTransaction(imuSPISettings);
  digitalWrite(IMU_CS, LOW);
  delayMicroseconds(1); // RP2350 setup time
  
  SPI1.transfer(0x33 | 0x80); 
  ...
  // 3. THE "UNLOCK" HANDSHAKE
  // Read Status0 (0x2E) to tell the sensor "I am finished, give me the next sample"
  imuReadReg(0x2E);
...
void loop() {
  ...
  if (imuReady) {
    ...
    // Serial.print(imuReadReg(0x2E), HEX);
    // Serial.print(", Statusint:");
    // Serial.println(imuReadReg(0x2D),HEX);
    // Serial.println(imuReadReg(REG_CTRL7),HEX);
